...
TEST_CASE("test_stabilizer_t_nn_dense", "[supreme]")
{
    // Try with environment variable
    // QRACK_QUNIT_SEPARABILITY_THRESHOLD=0.1464466
    // for clamping of single bit states to Pauli basis axes.
            ...
            for (d = 0; d < benchmarkDepth; d++) {
                ...
                for (row = 0; row < rowLen; row++) {
                    for (col = 0; col < colLen; col++) {
                        ...
                        const bool anti = gateRand = 2 * qReg->Rand();
    ...
    // Note - t: random permutation, f: random hadamard, t: log normal statistics, f: "qUniverse" 1qb 3-parameter
    // unitary
}
